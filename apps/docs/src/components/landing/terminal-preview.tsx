import { Shield, Zap } from 'lucide-react';

export function TerminalPreview() {
  return (
    <section className="py-12 px-6 md:px-12">
      <div className="max-w-5xl mx-auto relative group">
        {/* Glow effect */}
        <div className="absolute -inset-1 bg-gradient-to-r from-primary/20 to-transparent blur-2xl opacity-20 group-hover:opacity-30 transition-opacity" />

        {/* Terminal window */}
        <div className="relative bg-card border border-border rounded-2xl overflow-hidden shadow-2xl">
          {/* Terminal header */}
          <div className="h-10 border-b border-border bg-secondary/50 flex items-center px-4 gap-2">
            <div className="size-2.5 rounded-full bg-red-500/20 border border-red-500/40" />
            <div className="size-2.5 rounded-full bg-yellow-500/20 border border-yellow-500/40" />
            <div className="size-2.5 rounded-full bg-green-500/20 border border-green-500/40" />
            <div className="ml-4 flex items-center gap-2 text-[10px] font-mono text-muted-foreground">
              <Shield className="size-3" />
              docker-compose.yml
            </div>
          </div>

          {/* Code content */}
          <div className="p-6 md:p-8 font-mono text-sm leading-relaxed overflow-x-auto">
            <div className="flex gap-6">
              <span className="text-muted-foreground select-none w-6 text-right">
                1
              </span>
              <span>
                <span className="text-purple-400">services</span>
                <span className="text-foreground">:</span>
              </span>
            </div>
            <div className="flex gap-6">
              <span className="text-muted-foreground select-none w-6 text-right">
                2
              </span>
              <span>
                {'  '}
                <span className="text-blue-400">proxy</span>
                <span className="text-foreground">:</span>
              </span>
            </div>
            <div className="flex gap-6">
              <span className="text-muted-foreground select-none w-6 text-right">
                3
              </span>
              <span>
                {'    '}
                <span className="text-purple-400">image</span>
                <span className="text-foreground">: </span>
                <span className="text-primary">rproxy:latest</span>
              </span>
            </div>
            <div className="flex gap-6">
              <span className="text-muted-foreground select-none w-6 text-right">
                4
              </span>
              <span>
                {'    '}
                <span className="text-purple-400">ports</span>
                <span className="text-foreground">:</span>
              </span>
            </div>
            <div className="flex gap-6">
              <span className="text-muted-foreground select-none w-6 text-right">
                5
              </span>
              <span>
                {'      - '}
                <span className="text-primary">&quot;80:80&quot;</span>
              </span>
            </div>
            <div className="flex gap-6">
              <span className="text-muted-foreground select-none w-6 text-right">
                6
              </span>
              <span>
                {'      - '}
                <span className="text-primary">&quot;443:443&quot;</span>
              </span>
            </div>
            <div className="flex gap-6">
              <span className="text-muted-foreground select-none w-6 text-right">
                7
              </span>
              <span>
                {'    '}
                <span className="text-purple-400">environment</span>
                <span className="text-foreground">:</span>
              </span>
            </div>
            <div className="flex gap-6 bg-primary/10 -mx-6 md:-mx-8 px-6 md:px-8 border-l-4 border-primary">
              <span className="text-primary/50 select-none w-6 text-right">
                8
              </span>
              <span>
                {'      - '}
                <span className="text-foreground">RPROXY_UPSTREAM=</span>
                <span className="text-primary">nextjs:3000</span>
              </span>
            </div>
            <div className="flex gap-6 bg-primary/10 -mx-6 md:-mx-8 px-6 md:px-8 border-l-4 border-primary">
              <span className="text-primary/50 select-none w-6 text-right">
                9
              </span>
              <span>
                {'      - '}
                <span className="text-foreground">RPROXY_TLS_DOMAINS=</span>
                <span className="text-primary">example.com</span>
              </span>
            </div>
            <div className="flex gap-6">
              <span className="text-muted-foreground select-none w-6 text-right">
                10
              </span>
              <span>
                {'      - '}
                <span className="text-foreground">RPROXY_TLS_EMAIL=</span>
                <span className="text-primary">admin@example.com</span>
              </span>
            </div>
          </div>

          {/* Feature popup */}
          <div className="absolute bottom-6 right-6 p-4 md:p-6 bg-background/90 backdrop-blur border border-primary/20 rounded-xl shadow-2xl max-w-xs">
            <div className="flex items-center gap-3 mb-3">
              <div className="size-8 bg-primary rounded-lg flex items-center justify-center">
                <Zap className="size-4 text-primary-foreground" />
              </div>
              <span className="text-xs font-bold uppercase tracking-widest">
                Auto TLS
              </span>
            </div>
            <p className="text-[11px] text-muted-foreground leading-tight">
              Let&apos;s Encrypt certificates issued automatically. Zero
              configuration required.
            </p>
          </div>
        </div>
      </div>
    </section>
  );
}
